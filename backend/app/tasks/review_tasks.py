...
def process_pr_review(
    ...
    *,
    ...
    """
    Orchestrate PR review: fetch diff → clean → LLM review → post GitHub comments.

    Idempotency is enforced via Redis lock keyed by repository + head SHA.
    """
    ...
    try:
    ...
    except (GitHubServiceError, LLMReviewerError) as exc:
        logger.exception(
        ...
        logger.exception(
            "Unexpected PR review failure for %s#%s",
            repository_full_name,
            pr_number,
        )
        if self.request.retries >= self.max_retries:
...
def _persist_diff(
    ...
    """Store the fetched diff on the pull request record."""
    with session_factory() as session:
